...
selected_indices_path="./nads_results/llama3_code_weight/selected_indices.json"
split="train"
...
torchrun --master-port 29505 --nproc_per_node="$num_gpus" NADS-main/train_arithmetic.py \
  ...
  --new_instruction_field "$field_input" \
  --new_response_field "$field_output" \
  --candidate_data_path "$candidate_dataset" \
  --candidate_dataset_split "$split" \
  --candidate_instruction_field "$field_input" \
  --candidate_response_field "$field_output" \
